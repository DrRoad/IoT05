...
summary(model.glm)
h2o.saveModel(model.glm, path="./model", force = TRUE)

## Evaluate performance
yhat_train <- h2o.predict(model.glm, train)$predict
yhat_train <- as.factor(as.matrix(yhat_train))
levels_train <- unique(c(as.character(yhat_train),
                         as.character(y_train)))

yhat_test <- h2o.predict(model.glm, test)$predict
...
summary(model)
h2o.saveModel(model, path="./model", force = TRUE)

plot(model, timestep = "epochs", metric = "classification_error")
plot(model, timestep = "epochs", metric = "rmse")
plot(model, timestep = "epochs", metric = "auc")
...
#imp.vars <- c("x21", "x20","x95", "x85", "x13","x2","x96","x71","x4","x49")
imp.vars <- c("x21", "x20",  "x96","x71","x4","x49")
df <- train.df[c("t", imp.vars)]
df.melted <- melt(df, id = "t")
ggplot(data = df.melted, aes(x = t, y = value, color = variable))+geom_line()
ggplot(data = df.melted, aes(x = t, y = value, group = variable, colour = variable)) + geom_line() +
  facet_wrap(~ variable)
